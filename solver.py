...
def validity(board, num, pos):
    ...
    for i in range(len(board)):  # iteration through the 2D list
        if board[i][pos[1]] == num and pos[0] != i:  # check the column excluding current pos
            return False
...
def solve(board):  # use of backtrack algorithm
    ...
    for i in range(1, 10):  # loop through values 1 to 9
        if validity(board, i, (row, col)):  # check if the values are valid in the specific (row,col)
            ...
            if solve(board):  # solving board by calling the function recursively
                return True
            board[row][col] = 0  # condition when false, will rest the lop values and try new values
    return False
